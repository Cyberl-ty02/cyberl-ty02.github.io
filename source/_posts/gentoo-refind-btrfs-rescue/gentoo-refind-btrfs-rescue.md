...
## 后续更新：稳定系统中的版本化内核与 initramfs

上面的显式 `initrd=\initramfs-X.Y.Z-example.img` 保留了当时手工救援的真实做法。在 LiveCD 中诊断、自动匹配尚未可信时，把已核实的 initramfs 写清楚有助于排除歧义；这段救援过程本身并没有错。

系统稳定后，维护方式已经调整为版本化文件配对：
...
稳定系统中再确认 rEFInd 能按版本化文件名完成配对；如果只能从 LiveCD 维护，不应无检查地运行 `mkrlconf`，必要时仍可回到前文显式指定 initrd 的救援方式。
...
- [Gentoo OpenRC 桌面排错：中文字体、KDE 托盘与 PipeWire](/posts/gentoo-openrc-desktop-troubleshooting/gentoo-openrc-desktop-troubleshooting/)
- [Gentoo OpenRC 的时间同步、时区切换与 PostgreSQL 18 初始化](/posts/gentoo-chrony-timezone-postgresql/gentoo-chrony-timezone-postgresql/)
- [Gentoo 中 Firefox 与 CUDA 安装被旧 LLVM 构建失败阻塞的排查](/posts/gentoo-llvm-firefox-cuda-build-failure/gentoo-llvm-firefox-cuda-build-failure/)
